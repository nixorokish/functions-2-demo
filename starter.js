...
// going to write a higher order function that checks that the given
// variables are actually numbers. should work with all of them

const calculator = (num1, num2, callback) => {
    if(+num1 && +num2){ // + converts it to string
        num1 = +num1
        num2 = +num2
        return callback(num1, num2)
    } else {
        return `Math requires numbers`
    }
}
...
const copyArrToCamelCase = arr => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let num1 = 0; num1 < splitStr.length; num1++) {
            let word = splitStr[num1]
